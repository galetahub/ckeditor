CKEDITOR.dialog.add("scaytDialog", function(d) {
    var f = d.scayt,
        p = '\x3cp\x3e\x3cimg src\x3d"' + f.getLogo() + '" /\x3e\x3c/p\x3e\x3cp\x3e' + f.getLocal("version") + f.getVersion() + "\x3c/p\x3e\x3cp\x3e" + f.getLocal("text_copyrights") + "\x3c/p\x3e",
        q = CKEDITOR.document,
        l = {
            isChanged: function() {
        ...
        p = [{
                ...
                elements: [{
                    ...
                    children: function() {
                        ...
                        for (h in a) a = {
                            type: "checkbox"
                        }, a.id = h, a.label = f.getLocal(e[h]), b.push(a);
                        return b
                    }(),
                    onShow: function() {
                        this.getChild();
                        ...
                        children: [{
                            ...
                            onClick: function() {
                                var a = this.getDialog(),
                                    b = n,
                                    e = d.scayt,
                                    h = a.getContentElement("dictionaries", "dictionaryName").getValue();
                        ...
                        children: [{
                            ...
                            onClick: function() {
                                var a = this.getDialog(),
                                    b = d.scayt,
                                    e = n,
                                    h = a.getContentElement("dictionaries", "dictionaryName"),
                                    c = h.getValue();
    ...
    d.on("scaytUserDictionaryAction", function(a) {
        var b = SCAYT.prototype.UILib,
            e = a.data.dialog,
            d = e.getContentElement("dictionaries", "dictionaryNote").getElement(),
            c = a.editor.scayt,
            g;
        void 0 === a.data.error ? (g = c.getLocal("message_success_" + a.data.command + "Dic"), g = g.replace("%s", a.data.name), d.setText(g), b.css(d.$, {
            color: "blue"
        })) : ("" === a.data.name ? d.setText(c.getLocal("message_info_emptyDic")) : (g = c.getLocal("message_error_" + a.data.command + "Dic"), g = g.replace("%s", a.data.name), d.setText(g)), b.css(d.$, {
                color: "red"
            }),
    ...
    var n = {
        ...
        onShow: function() {
            ...
            if (0 != d.config.scayt_uiTabs[2]) {
                var a = d.scayt,
                    ...
                    e = this.getContentElement("dictionaries", "existDic").getElement().getParent(),
        ...
        getChangedOption: function() {
            ...
            if (1 == d.config.scayt_uiTabs[0])
                for (var b = this.getContentElement("options", "scaytOptions").getChild(), e = 0; e < b.length; e++) b[e].isChanged() && (a[b[e].id] = b[e].getValue());
        ...
        buildRadioInputs: function(a, b, e) {
            var h = new CKEDITOR.dom.element("div"),
                c = "scaytLang_" + d.name + "_" + b,
                g = CKEDITOR.dom.element.createFromHtml('\x3cinput id\x3d"' + c + '" type\x3d"radio"  value\x3d"' + b + '" name\x3d"scayt_lang" /\x3e'),
            ...
            e && d.config.grayt_autoStartup && f.setStyles({
                color: "#02b620"
            });
            h.append(g);
            h.append(f);
            b === m.getLang() && (g.setAttribute("checked", !0), g.setAttribute("defaultChecked", "defaultChecked"));
            return h
        },
        renderLangList: function(a) {
            var b = a.find("#left-col-" + d.name).getItem(0);
            a = a.find("#right-col-" + d.name).getItem(0);
            var e = f.getScaytLangList(),
                h = f.getGraytLangList(),
                c = {},
                g = [],
                l = 0,
                m = !1,
